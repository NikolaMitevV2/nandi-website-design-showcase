...
const PricingSection = () => {
  ...
  const getFeatureIcon = (feature) => {
    const iconMap = {
      ...
      "Basic website audit": Globe,
      "Strategy recommendations": Target,
      "Project timeline": Calendar,
      "Cost estimation": DollarSign,
      "Custom design & development": Palette,
      "Mobile-responsive design": Smartphone,
      "SEO optimization": Search,
      "Content management system": Settings,
      "3 months support": Headphones,
      "Performance optimization": Zap,
      "Analytics integration": BarChart3,
      "Everything in Professional": Star,
      "Advanced functionality": Settings,
      "Custom integrations": Building,
      "Priority support": Shield,
      "12 months maintenance": Headphones,
      "Dedicated project manager": Users,
      "Training sessions": BookOpen,
      "Scalable architecture": Building,
    };

    const IconComponent = iconMap[feature] || MessageCircle;
    return (
      <IconComponent
        size={20}
        className="text-space-purple-light mt-0.5 flex-shrink-0"
      />
    );
  };
    ...
    <section className="py-32 px-6 relative overflow-hidden">
      <div ref={sectionRef} className="container mx-auto max-w-7xl">
        <div className="text-center mb-20">
          ...
          >
            Choose the perfect plan that aligns with your business goals and
            budget
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <div
              key={index}
              className={`animate-on-scroll card-hover p-8 rounded-3xl relative ${
                plan.recommended
                  ? "scale-110 border-2 border-space-purple "
                  : ""
              }`}
              style={{
              ...
              <ul className="space-y-4 mb-8">
                {plan.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start space-x-3">
                    ...
                    <span className="font-lato font-light text-body text-gray-50">
                      {feature}
                    </span>
                  </li>
                ))}
      ...
      <div
        className="orbital-decoration w-36 h-36 top-20 right-10 opacity-10"
        style={{
          animationDelay: "2s",
        }}
      />
      ...
      <div
        className="orbital-decoration w-24 h-24 bottom-32 left-20 opacity-15"
        style={{
          animationDelay: "7s",
        }}
      />
